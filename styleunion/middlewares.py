...
class CustomRetryMiddleware(RetryMiddleware):
    ...
    def __init__(self, settings):
        ...
        self.retry_http_codes = set(int(x) for x in settings.getlist('RETRY_HTTP_CODES', [429, 503, 504]))
        ...
        if response.status in self.retry_http_codes:
            ...
            if response.status == 429:
                ...
                if retry_times <= self.max_retry_times:
                    ...
                    backoff_time = min(10 * (2 ** retry_times), 300)  # Max 5 minutes
                    jitter = random.uniform(0, 5)  # Add random jitter
                    wait_time = backoff_time + jitter
                    
                    spider.logger.warning(
                        f"Rate limited (429) on {request.url}. "
                        f"Waiting {wait_time:.1f} seconds before retry {retry_times}/{self.max_retry_times}"
                    )
                    ...
                    spider.logger.error(
                        f"Gave up retrying {request.url} after {retry_times} times (429 rate limit)"
                    )
